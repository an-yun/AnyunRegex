...
namespace anyun_regex
{
	...
	bool NFA::find()
	{
		...
		while (!is_find && start <= text_length)
		{
			...
			for (size_t index = start;!state.empty() && index < text_length; index++)
			{
				...
				get_final = get_next_state(state,text,index,matcher);
	...
	bool NFA::get_sigma_closure(list<size_t>& source)
	{
		...
		for (list<size_t>::iterator b = source.begin(), e = source.end(); b != e; b++)
		{
			...
			visited[id] = true;
			node_ids.push(id);
			if (digraph.nodes[id]->get_type() == END_DIRECTEDNODE)get_final = true;
		}
		...
		while (!node_ids.empty())
		{
			...
			{
				size_t edge_id = *b;
				size_t end_node_id = digraph.edges[edge_id]->get_end_node_id(matcher);
				if ((!visited[end_node_id]) && digraph.edges[edge_id]->get_type() ==SIGMA_DIRECTEDEDGE)
				{
					...
					if (digraph.nodes[end_node_id]->get_type() == END_DIRECTEDNODE)get_final = true;
				}
			}
	...
	bool NFA::get_next_state(list<size_t> & state, const string &text, size_t index, Matcher &matcher)
	{
		...
		for (list<size_t>::iterator b = state.begin(), e = state.end(); b != e; b++)
		{
			...
			for (vector<size_t>::const_iterator edge_b = out_edges.begin(), edge_e = out_edges.cend(); edge_b != edge_e; edge_b++)
				if (edges[*edge_b]->accept(text,index,matcher))next_state.insert(edges[*edge_b]->get_end_node_id(matcher));
		}
		state.clear();
